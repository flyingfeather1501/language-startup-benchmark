...
report_success () {
    # report_success <language> <output> <time>
    cat <<JSON
{"status":"success","report":{"language":"$1","output":"$2","time":$3,"unit":"second"}}
JSON
}
...
report_time () {
    # report_time <language> <command> <args> ...
    TIMEFORMAT="%3R"
    language="$1"; shift 1
    tmp_time=$(mktemp)
    ...
    report_success "$language" "$output" "$time_seconds"
